...
void setup_layer(struct layer *layer, int Batch, int In, int Out) {
	...
	float (*weights)[In] = (float *)layer->weights;
...
__global__ void dot_fwd_cblas(float *act, float *weights, float *zout, int Batch, int In, int Out) {
    // row-major order
    // act[Batch][In] => A
    // weights[Out][In] => B
    // zout[Batch][Out] => C
...
void softmax(int batch, int b, float C[batch][b], float A[batch][b]) {
		...
		float exp_new_value[b];
			...
			// Subtract largest value for stability 
...
void forward_pass(struct layer *ll, int lastlayer) {
	...
	float (*z_list)[Out] = (float *)ll->z_list;
	float (*fwd)[Out] = (float *)ll->fwd;

	//float *act = (float *)ll->a_list;
	//float *zout = (float *)ll->z_list;
	//float *wts = (float *)ll->weights;

	dot_fwd_cblas((float *)ll->a_list, (float *)ll->weights, 
	              (float *)ll->z_list, Batch, In, Out);
...
void bwd_pass(struct layer *ll, int lastlayer) {
	...
	const int In = ll->in;
	const int Out = ll->out;
	float (*deltas)[Out] = (float *)ll->deltas;
	float (*z_list)[Out] = (float *)ll->z_list;
	...
	dot_bwd_cblas((float *)ll->deltas, (float *)ll->weights, (float *)ll->bwd, Batch, In, Out);
...
void weightupd(struct layer *ll, int num, int step) {
	...
	float *act = (float *)ll->a_list;
	float *del = (float *)ll->deltas;
	float *grd = (float *)ll->w_grad;
	float *wts = (float *)ll->weights;
...
int train(struct network *nn, int nstep, int ntrain, float *ztrain, float *zlabel)
{
	const int L = nn->L;
	...
	float (*images)[first->in] = (float *)first->a_list;
    ...
    for (int j=0, jn=nstep; j<jn; ++j) {
        ...
        softmax(batch, nclass, p, (float *)last->fwd);

		// loss
	    int match = loss(batch,nclass, (float*)last->deltas, p, labels);
		tot_match += match;
...
int test(struct network *nn, int nstep, int ntest, float *ztest, float *ztestlabel)
{
	int test_match = 0;
	...
	float (*testImages)[first->in] = (float *)first->a_list;
    ...
    for (int j=0, jn=nstep; j<jn; ++j) {
		...
		// loss
	    int matching = loss(batch,nclass, (float*)last->deltas, p, t_label);
		test_match += matching;
	}
...
int main(const int argc, const char** argv){
	...
	printf("layres = %d, batchsz = %d, seed = %d \n", 
		nl, nb, seed);
	...
	// Load MNIST data
    int numberOfImages;
    float *images = readMNISTImages(TRAIN_IMG, &numberOfImages, &inputSize);
    int numberOfLabels;
    float *labels = readMNISTLabels(TRAIN_LBL, &numberOfLabels);

	int numberOfTestImages, testImageSize;
	float *testImages = readMNISTImages(TEST_IMG, &numberOfTestImages, &testImageSize);
	int numberOfTestLabels;
	float *testLabels = readMNISTLabels(TEST_LBL, &numberOfTestLabels);

	dim3 blocks(10); // Number of blocks
	dim3 threadsPerBlock(256); // Number of threads per block
	...
	struct network *nn_d;
	float *images_d, *labels_d;
	int *batch_d;
	assert(cudaMalloc((void **) &nn_d, sizeof(*nn))==cudaSuccess);
	assert(cudaMalloc((void **) &images_d, inputSize*imgs*sizeof(float))==cudaSuccess);
	assert(cudaMalloc((void **) &labels_d, imgs*sizeof(float))==cudaSuccess);
	assert(cudaMalloc((void **) &batch_d, nb*sizeof(int))==cudaSuccess);

	assert(cudaMemcpy(nn_d, nn, sizeof(*nn), cudaMemcpyHostToDevice)==cudaSuccess);
	assert(cudaMemcpy(images_d, images, inputSize*imgs*sizeof(float), cudaMemcpyHostToDevice)==cudaSuccess);
	assert(cudaMemcpy(labels_d, labels, imgs*sizeof(float), cudaMemcpyHostToDevice)==cudaSuccess);
	...
	for (int i=0, n=ne; i<n; ++i) {
		...
		// training
		train<<<blocks, threadsPerBlock>>>(nn_d, imgs/nb, numberOfImages, images_d, labels_d);
			
		cudaEventRecord( stop_d, 0 ); // record cuda finish time
		cudaEventSynchronize( stop_d );
		cudaEventElapsedTime( &time_device, start_d, stop_d );
	...
	int test_match = test(nn, test_imgs/nb, numberOfTestImages, testImages, testLabels);
	float test_acc = ((float)test_match)/((float)numberOfTestImages) * 100;
	...
	cudaFree(nn_d); cudaFree(images_d); cudaFree(labels_d); cudaFree(batch_d);
	return 0;
