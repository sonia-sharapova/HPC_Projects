...
void dot_fwd_cblas(float *act, float *weights, float *zout, int Batch, int In, int Out) {
    ...
    // Since we're working with row-major order and no transposition of the matrices
    // act[Batch][In] => A
    // weights[Out][In] => B, needs to be transposed to match the expected order
    // zout[Batch][Out] => C
...
void dot_bwd_cblas(float *deltas, float *weights, float *bwdout, int Batch, int In, int Out) {
    ...
    // Since we're working with row-major order and no transposition of the matrices
    // deltas[Batch][Out] => A
    // weights[Out][In] => B, but since we want the gradient with respect to the input, it needs not to be transposed
    // bwdout[Batch][In] => C
...
void dot_grd_cblas(float *act, float *del, float *grd, int Batch, int In, int Out) {
    ...
    // Since we're working with row-major order
    // act[Batch][In] => A, but needs to be transposed to match the multiplication
    // del[Batch][Out] => B
    // grd[Out][In] => C
...
void bwd_pass(struct layer *ll) {
	...
	//float (*b_grad)[Out] = (void *)ll->b_grad;
	//float (*w_grad)[Out] = (void *)ll->w_grad;
...
int test(struct network *nn, int nstep, int ntest, void *ztest, void *ztestlabel)
{
    ...
    for (int j=0, jn=nstep; j<jn; ++j) {
	    ...
	    int matching = loss(batch,nclass, (void*)last->deltas, (void *)last->fwd, t_label);
		test_match += matching;
	}
...
int main(const int argc, const char** argv){
	...
	for (int i=0, n=ne; i<n; ++i) {
		clock_t start = clock();
		int match = train(nn, imgs/nb, numberOfImages, images, labels);
        float acc = ((float)match)/((float)imgs) * 100;
		printf("epoch %d: %d/%d correct, acc: %2.2f, alpha: %f\n", 
		    i, match, imgs, acc, alpha);
		clock_t end = clock();
